...
class WeightedEdge(Edge):
	...
	def __str__(self):
		return '{}->{} ({})'.format(self.src, self.dest, self.total_distance)  # override the print statement
...
class Digraph(object):
	...
	def add_node(self, node):
		...
		if node in self.nodes:
			...
			self.edges[node] = []
			self.nodes = set(list(self.edges.keys()))
	...
	def add_edge(self, edge):
		...
		in the graph."""
		src = edge.get_source()
		dest = edge.get_destination()  # get the name of the start and the end of a edge
		# raise error if either of the nodes associated with the edge is not in the graph
		if not (src in self.edges and dest in self.edges):
			raise ValueError('Node not in graph')
		self.edges[src].append(edge)  # add edge into dictionary of edges if no any error
