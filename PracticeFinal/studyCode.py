...
def findBalancePoint(trainingSet, label, epsilon):
	"""
	  assumes: trainingSet: a set (of size > 1) of examples for building
				   a model
			   label: a label
			   epsilon: a float between 0 and 1
	  returns the probability cutoff such that the difference between
			   specificity and sensitivity of assigning the label, using
			   a logistic regression model, is less than epsilon.
	"""
	...
	while not ifend:
		prob = (pmin + pmax) / 2
		truePos, falsePos, trueNeg, falseNeg = leaveOneOut(trainingSet, lambda train, test: \
			lr(train, test, label, prob), False)
		if abs(specificity(trueNeg, falsePos) - sensitivity(truePos, falseNeg)) <= epsilon:
			ifend = True
		else:
			print(prob)
			if specificity(trueNeg, falsePos) > sensitivity(truePos, falseNeg):
				pmax = prob
			else:
				pmin = prob
...
# def test(examples, label, prob, toPrint):
...
# print(test(examples, 'Survived', 0.5, True))

examples = buildTitanicExamples('TitanicPassengers.txt')
print(round(findBalancePoint(examples, 'Survived', 0.01), 2))