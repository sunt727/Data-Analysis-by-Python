...
if __name__ == '__main__':
	...
	# def dpMakeChange(cow_weights, target, solutions, memo):
	# 	for cents in range(target + 1):
	# 		coinCount = cents
	# 		newCoin = 3
	# 		for j in [c for c in cow_weights if c >= cents]:
	# 			if solutions[cents - j] + 1 < coinCount:
	# 				coinCount = solutions[cents - j] + 1
	# 				newCoin = j
	# 		solutions[cents] = coinCount
	# 		memo[cents] = newCoin
